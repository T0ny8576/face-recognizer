...
log_handler = logging.FileHandler("Last_Log.txt")
...
class GabrielServer(ABC):
    ...
    async def send_result_wrapper(
        ...
        to_client_payload = to_client.SerializeToString()
        logger.warning('ServerBytes,{},{},{}'.format(address, frame_id, len(to_client_payload)))

        return await self._send_via_transport(address, to_client_payload)
