...
class WebsocketServer(GabrielServer):
    ...
    async def _consumer(self, websocket, client):
        ...
        async for raw_input in websocket:
            ...
            logger.warning('ClientBytes,{},{},{}'.format(address, from_client.frame_id, len(raw_input)))
            ...
            if status == ResultWrapper.Status.SUCCESS:
                ...
                ###############################
                logger.warning('Arrive,{},{},{}'.format(address, from_client.frame_id, int(time.time() * 1000.)))
